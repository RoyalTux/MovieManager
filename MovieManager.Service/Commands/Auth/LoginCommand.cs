...
namespace MovieManager.Service.Commands.Auth
{
    ...
    public class LoginCommandHandler : TokenApiBase, IRequestHandler<LoginCommand, LoginResponse>
    {
        ...
        {
            var user = await _userManager.FindByNameAsync(request.UserName) ?? throw new Exception("Invalid username or password");

            if (!await _userManager.CheckPasswordAsync(user, request.Password)) throw new Exception("Invalid username or password");
